        ...
        private void FillTable()
        {
            foreach (var node in _nodes.Select(e => e.Key))
            {
                ...
                foreach (var element in node.Kernel)
                {
                    ...
                    {
                        //if (_actions[_nodes[node]][symbol] != null)
                        //{
                        //    var error = new Tuple<ActionType, ActionType>(_actions[_nodes[node]][symbol].Item1, ActionType.Shift);
                        //    _errors.Add(error);
                        //    if (OnError != null)
                        //        OnError(error);
                        //}
                        ...
                        _actions[_nodes[node]][symbol] = new Tuple<ActionType, int>(ActionType.Reduce, _idByProduction[element.Production]);//Reduce
                    }
                }
            }
        }
